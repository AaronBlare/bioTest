...
---
# -------------------------------------------------------------------
# How to run this SeSAMe EPIC v1 DNAm QC pipeline on a new dataset
# -------------------------------------------------------------------
# 1) Required input files and structure
#    - A sample sheet CSV with at least:
#        * SampleID: unique sample identifier
#        * cohort:  cohort label (e.g. "NS", "SS", "combined"), can be 1 level
#        * region:  region label (e.g. "Region1", "Region2"), can be 1 level
#        * sex:     reported sex (e.g. "M"/"F")
#      You can add extra columns (age, batch, plate, etc.), they will be
#      carried through into the QC tables.
#    - A directory containing compressed IDAT files (*.idat.gz) for all
#      samples in the sample sheet. Each sample must have one Grn and one
#      Red IDAT file sharing a common prefix.
#
# 2) What you need to change for a new dataset
#    - samplesheet_path: point to the new sample sheet CSV.
#    - idat_dir:         point to the directory with the new IDATs.
#    - Optional: update expected cohort / region levels in any plots
#      or labels if you want nicer facet titles, but the code does not
#      require this.
#
# 3) What stays the same across datasets
#    - The SeSAMe processing and QC thresholds:
#        * Detection failure thresholds (soft and hard).
#        * Mean intensity threshold (5th percentile).
#        * Sex inference and sex-mismatch checking.
#    - The EPIC v1 platform and known-bad probe filters:
#        * Pidsley-derived cross-reactive and SNP-affected probe lists.
#        * Probe-level missingness > 5% filter.
#    - All output file names (they will be written in the working
#      directory for each dataset run).
#
# 4) Main outputs produced per dataset
#    - EPICv1_sesame_QC_table.csv
#        * Per-sample QC metrics, flags, inferred sex, etc.
#    - EPICv1_sesame_QCoverallsummary.csv
#        * Overall QC counts (pass/fail, reasons) for the dataset.
#    - EPICv1_sesame_QCbycohortregion.csv
#        * QC summaries stratified by cohort and region.
#    - EPICv1_sesame_masked_beta_matrix.rds
#        * SeSAMe beta matrix with probe-level masking applied.
#    - EPICv1_sesame_masked_beta_matrix_probe_filtered.rds / .csv
#        * Final beta matrix after excluding non-CpG probes,
#          probes with >5% missingness, and known-bad probes.
#    - EPICv1_sesame_probe_QC_table.csv
#        * Probe-level QC metrics (missingness, known-bad status, keep flag).
#    - EPICv1_sesame_probe_QC_overview.csv
#        * Small summary table of probe counts by filter category.
#    - Plots/ directory
#        * PDFs/PNGs of QC histograms and per-sample beta density plots.
#
# 5) Typical workflow across multiple datasets
#    - For each dataset (e.g. different regions or cohorts):
#        * Update samplesheet_path and idat_dir.
#        * Knit the Rmd to run the full pipeline.
#        * Inspect the DNAm QC REPORT at the end and key plots.
#        * Keep the per-dataset QC tables and filtered beta matrices.
#    - Later, to create a combined dataset:
#        * Build a combined sample sheet (with a consistent SampleID,
#          cohort and region coding across datasets).
#        * Point samplesheet_path and idat_dir to the combined inputs.
#        * Rerun this same Rmd once to obtain a joint QC report and a
#          combined filtered beta matrix.
#
# 6) Reproducibility notes
#    - This script assumes the Illumina MethylationEPIC v1.0 platform.
#    - Known-bad probe filters are taken from Pidsley et al. for EPIC.
#    - If you change thresholds or probe filters, document the changes
#      in the DNAm QC REPORT section before sharing results.
# -------------------------------------------------------------------
## 1. Setup
...
## 2. Read sample sheet and match IDATs
...
sample_sheet_path <- "main_sample_sheet.csv"
...
## 4. Run SeSAMe QC in parallel (NN test run)
...
## 5. Build extended QC table and flags
...
fwrite(ss_qc_full, "EPICv1_sesame_QC_table.csv")
...
fwrite(qc_overall, "EPICv1_sesame_QC_overall_summary.csv")
fwrite(qc_by_cohort_region, "EPICv1_sesame_QC_by_cohort_region.csv")
...
## 6b. Extra background outlier flags (OOB metrics)

```{r}
## Extra background outlier flags (OOB metrics)
...
## 7. Density plots using masked betas
...
## 9. SNP QC and SeSAMe diagnostic plots
...
if (length(beta_list_ok) < 2) {
  ...
  if (length(rs_probes) < 10) {
    ...
    saveRDS(snp_beta_mat, file = "EPICv1_sesame_SNP_beta_matrix.rds")
    fwrite(
      data.table(Probe_ID = rownames(snp_beta_mat), snp_beta_mat),
      "EPICv1_sesame_SNP_beta_matrix.csv"
    )
    ...
    fwrite(snp_qc_dt, "EPICv1_sesame_SNPcheck_table.csv")
  }
}
...
## 9b. SNP-based correlation heatmap and clustering
...
## 9c. SNP-based relatedness table (pairwise similarities)
...
fwrite(related_dt,
       file = "EPICv1_sesame_SNP_relatedness_table.csv")
...
## 10. Final masked beta matrix after sample filtering
...
## 4. Save RDS and CSV
saveRDS(beta_mat_masked, file = "EPICv1_sesame_masked_beta_matrix.rds")

fwrite(
  data.table(Probe_ID = rownames(beta_mat_masked), beta_mat_masked),
  "EPICv1_sesame_masked_beta_matrix.csv"
)
...
## 10b. Create CSV of known bad EPIC probe IDs from published resources

```{r}
## 10b (fixed again). Create CSV of known bad EPIC probe IDs from published resources

library(data.table)

bad_probe_dir <- "probe_filters"
...
## 10c. Probe-level QC filtering of final beta matrix
...
if (!exists("beta_mat_masked") || is.null(beta_mat_masked)) {
  if (!file.exists("EPICv1_sesame_masked_beta_matrix.rds")) {
    stop("EPICv1_sesame_masked_beta_matrix.rds not found.")
  }
  beta_mat_masked <- readRDS("EPICv1_sesame_masked_beta_matrix.rds")
}
...
bad_probe_file <- file.path("probe_filters", "EPIC_known_bad_probes_ids.csv")
...
## 6. Save outputs
saveRDS(beta_mat_probe_filtered, "EPICv1_sesame_masked_beta_matrix_probe_filtered.rds")

fwrite(
  data.table(Probe_ID = rownames(beta_mat_probe_filtered), beta_mat_probe_filtered),
  "EPICv1_sesame_masked_beta_matrix_probe_filtered.csv"
)

fwrite(probe_qc_dt, "EPICv1_sesame_probe_QC_table.csv")
...
## 10d. PROBE-LEVEL QC OVERVIEW

```{r}
## PROBE-LEVEL QC OVERVIEW (compact table for reports)
...
if (!exists("probe_qc_dt") || !exists("beta_mat_masked") || !exists("beta_mat_probe_filtered")) {
  ...
  fwrite(
    probe_QC_overview,
    file = "EPICv1_sesame_probe_QC_overview.csv"
  )
...
## 11. QC report: sample counts and reasons

```{r}
## Enhanced QC report + SNP-relatedness summary
