...
sample_sheet_path <- "D:/Yandex.Disk/bbd/novosib/nn/idat/main_sample_sheet.csv"
...
required_cols <- c(
  "Sample_ID", "cohort", "region", "sex", "age",
  "basename", "Sentrix_Position", "Sentrix_ID"
)
...
ss[, cohort := as.character(cohort)]
ss[, region := as.character(region)]
...
process_one_sample <- function(id_prefix, sample_id, idat_dir, platform = "EPIC", verbose = TRUE) {
  start_time <- Sys.time()
...
fwrite(ss_qc_full, "C:/Git/bioTest/r/EPICv1_sesame_QC_table.csv")
...
qc_by_cohort_region <- ss_qc_full[, .(
  n_total = .N,
  n_fail_any = sum(qc_fail, na.rm = TRUE),
  n_pass_all = sum(qc_fail == FALSE, na.rm = TRUE),
  n_fail_frac_soft = sum(fail_frac_soft, na.rm = TRUE),
  ...
  n_fail_status = sum(fail_status, na.rm = TRUE),
  n_fail_sex = sum(fail_sex, na.rm = TRUE),
  perc_fail_any = round(100 * sum(qc_fail, na.rm = TRUE) / .N, 1),
  frac_failed_mean = mean(frac_failed, na.rm = TRUE),
  frac_failed_median = median(frac_failed, na.rm = TRUE),
  mean_intensity_mean = mean(mean_intensity, na.rm = TRUE),
  mean_intensity_median = median(mean_intensity, na.rm = TRUE),
  bs_gct_score_mean = mean(bs_gct_score, na.rm = TRUE),
  bs_gct_score_median = median(bs_gct_score, na.rm = TRUE)
), by = .(cohort, region)]

fwrite(qc_overall, "C:/Git/bioTest/r/EPICv1_sesame_QC_overall_summary.csv")
fwrite(qc_by_cohort_region, "C:/Git/bioTest/r/EPICv1_sesame_QC_by_cohort_region.csv")

message("Overall QC summary:")
print(qc_overall)

message("QC summary by cohort and region:")
print(qc_by_cohort_region)
...
beta_long <- merge(
  beta_long,
  ss_qc_full[, .(Sample_ID, cohort, region)],
  by = "Sample_ID",
  all.x = TRUE
)
...
p_all <- ggplot(beta_long, aes(x = Beta, group = Sample_ID, colour = cohort)) +
...
p_region <- ggplot(beta_long, aes(x = Beta, group = Sample_ID, colour = cohort)) +
  geom_density(alpha = 0.2, linewidth = 0.3) +
  scale_x_continuous(limits = c(0, 1)) +
  facet_wrap(~ region) +
  labs(
    title = "Per-sample Beta-value density by region (masked betas, QC-passed)",
    x = "Beta-value",
    y = "Density",
    colour = "Cohort"
  ) +
...
plots_dir <- "C:/Git/bioTest/r/plots"
...
if (exists("p_region")) {
  ggsave(file.path(plots_dir, "DNAm_beta_density_by_region.pdf"),
         plot = p_region, width = 8, height = 5)
}
...
if (length(beta_list_ok) < 2) {
  ...
  if (length(rs_probes) < 10) {
    ...
    saveRDS(snp_beta_mat, file = "C:/Git/bioTest/r/EPICv1_sesame_SNP_beta_matrix.rds")
    fwrite(
      data.table(Probe_ID = rownames(snp_beta_mat), snp_beta_mat),
      "C:/Git/bioTest/r/EPICv1_sesame_SNP_beta_matrix.csv"
    )
    ...
    fwrite(snp_qc_dt, "C:/Git/bioTest/r/EPICv1_sesame_SNPcheck_table.csv")
  }
}
...
annot <- ss_qc_full[, .(Sample_ID, cohort, region)]
...
setnames(cor_dt, c("cohort", "region"), c("cohort1", "region1"))
...
setnames(cor_dt, c("cohort", "region"), c("cohort2", "region2"))
...
if (exists("ss_qc_full")) {
  ...
  setnames(related_dt, c("cohort", "region"), c("cohort1", "region1"))

  related_dt <- merge(
    related_dt, annot,
    by.x = "Sample2", by.y = "Sample_ID", all.x = TRUE
  )
  setnames(related_dt, c("cohort", "region"), c("cohort2", "region2"))
}
...
fwrite(related_dt,
       file = "C:/Git/bioTest/r/EPICv1_sesame_SNP_relatedness_table.csv")

cat("\nSNP-based relatedness table written to EPICv1_sesame_SNP_relatedness_table.csv\n")
...
## 4. Save RDS and CSV
saveRDS(beta_mat_masked, file = "C:/Git/bioTest/r/EPICv1_sesame_masked_beta_matrix.rds")

fwrite(
  data.table(Probe_ID = rownames(beta_mat_masked), beta_mat_masked),
  "C:/Git/bioTest/r/EPICv1_sesame_masked_beta_matrix.csv"
)

message(
  "Final masked beta matrix written: ",
  nrow(beta_mat_masked), " probes × ", ncol(beta_mat_masked), " samples.\n",
  "Files: EPICv1_sesame_masked_beta_matrix.rds and .csv"
)

library(data.table)

bad_probe_dir <- "C:/Git/bioTest/r/probe_filters"
if (!dir.exists(bad_probe_dir)) dir.create(bad_probe_dir)

base_url <- "https://static-content.springer.com/esm/art%3A10.1186%2Fs13059-016-1066-1/MediaObjects/"

files_to_get <- c(
  cross_reactive = "13059_2016_1066_MOESM1_ESM.csv",
  snp_cpg        = "13059_2016_1066_MOESM4_ESM.csv",
  snp_sbe        = "13059_2016_1066_MOESM5_ESM.csv",
  snp_probe_body = "13059_2016_1066_MOESM6_ESM.csv"
)
...
safe_download <- function(url, destfile) {
  ...
  if (inherits(status, "error") || !file.exists(destfile) || is.na(file.size(destfile)) || file.size(destfile) == 0) {
    stop(
      "Failed to download or empty file: ", destfile,
      "\nURL: ", url,
      "\nPlease check internet access or download manually into: ", dirname(destfile)
    )
  }
}

## 1. Download files
for (nm in names(files_to_get)) {
  fname <- files_to_get[[nm]]
  dest  <- file.path(bad_probe_dir, fname)
  url   <- paste0(base_url, fname)

  if (!file.exists(dest) || is.na(file.size(dest)) || file.size(dest) == 0) {
    safe_download(url, dest)
  }
}

## 2. Define paths explicitly
cross_file <- file.path(bad_probe_dir, files_to_get[["cross_reactive"]])
cpg_file   <- file.path(bad_probe_dir, files_to_get[["snp_cpg"]])
sbe_file   <- file.path(bad_probe_dir, files_to_get[["snp_sbe"]])
body_file  <- file.path(bad_probe_dir, files_to_get[["snp_probe_body"]])

## Optional sanity check
print(data.table(
  label = c("cross_reactive", "snp_cpg", "snp_sbe", "snp_probe_body"),
  path  = c(cross_file, cpg_file, sbe_file, body_file),
  exists = file.exists(c(cross_file, cpg_file, sbe_file, body_file)),
  size   = file.size(c(cross_file, cpg_file, sbe_file, body_file))
))

## 3. Read files
cross_reactive <- fread(cross_file)
snp_cpg        <- fread(cpg_file)
snp_sbe        <- fread(sbe_file)
snp_probe_body <- fread(body_file)

## 4. Extract probe IDs
cross_ids <- unique(as.character(cross_reactive[[1]]))

if (!("PROBE" %in% names(snp_cpg))) stop("Column PROBE not found in snp_cpg file.")
if (!("PROBE" %in% names(snp_sbe))) stop("Column PROBE not found in snp_sbe file.")

snp_cpg_ids <- unique(as.character(snp_cpg$PROBE))
snp_sbe_ids <- unique(as.character(snp_sbe$PROBE))

req_cols <- c("PROBE", "VARIANT_START", "VARIANT_END", "MAPINFO")
if (!all(req_cols %in% names(snp_probe_body))) {
  stop("Unexpected columns in probe-body SNP file. Found: ",
       paste(names(snp_probe_body), collapse = ", "),
       "\nExpected at least: ", paste(req_cols, collapse = ", "))
}

dist_start <- abs(snp_probe_body$VARIANT_START - snp_probe_body$MAPINFO)
dist_end   <- abs(snp_probe_body$VARIANT_END - snp_probe_body$MAPINFO)
dist_min   <- pmin(dist_start, dist_end, na.rm = TRUE)

snp_body_ids <- unique(as.character(snp_probe_body$PROBE[dist_min < 10]))

## 5. Combine
bad_probe_dt <- unique(rbindlist(list(
  data.table(Probe_ID = cross_ids,   bad_probe_reason = "cross_reactive"),
  data.table(Probe_ID = snp_cpg_ids, bad_probe_reason = "snp_at_cpg"),
  data.table(Probe_ID = snp_sbe_ids, bad_probe_reason = "snp_at_sbe"),
  data.table(Probe_ID = snp_body_ids, bad_probe_reason = "snp_in_probe_body_within_10bp")
...
## 6. Save
fwrite(bad_probe_dt, file.path(bad_probe_dir, "EPIC_known_bad_probes_annotated.csv"))
fwrite(bad_probe_ids_dt, file.path(bad_probe_dir, "EPIC_known_bad_probes_ids.csv"))

cat("\nKnown bad EPIC probe list created.\n")
cat("Annotated file: probe_filters/EPIC_known_bad_probes_annotated.csv\n")
cat("Simple ID file: probe_filters/EPIC_known_bad_probes_ids.csv\n")
...
if (!exists("beta_mat_masked") || is.null(beta_mat_masked)) {
  if (!file.exists("C:/Git/bioTest/r/EPICv1_sesame_masked_beta_matrix.rds")) {
    stop("EPICv1_sesame_masked_beta_matrix.rds not found.")
  }
  beta_mat_masked <- readRDS("C:/Git/bioTest/r/EPICv1_sesame_masked_beta_matrix.rds")
}
...
bad_probe_file <- file.path("C:/Git/bioTest/r/probe_filters", "EPIC_known_bad_probes_ids.csv")
...
## 6. Save outputs
saveRDS(beta_mat_probe_filtered, "C:/Git/bioTest/r/EPICv1_sesame_masked_beta_matrix_probe_filtered.rds")

fwrite(
  data.table(Probe_ID = rownames(beta_mat_probe_filtered), beta_mat_probe_filtered),
  "C:/Git/bioTest/r/EPICv1_sesame_masked_beta_matrix_probe_filtered.csv"
)

fwrite(probe_qc_dt, "C:/Git/bioTest/r/EPICv1_sesame_probe_QC_table.csv")
...
cat("Files written:\n")
cat("  - EPICv1_sesame_masked_beta_matrix_probe_filtered.rds\n")
cat("  - EPICv1_sesame_masked_beta_matrix_probe_filtered.csv\n")
cat("  - EPICv1_sesame_probe_QC_table.csv\n")
...
if (!exists("probe_qc_dt") || !exists("beta_mat_masked") || !exists("beta_mat_probe_filtered")) {
  ...
  fwrite(
    probe_QC_overview,
    file = "C:/Git/bioTest/r/EPICv1_sesame_probe_QC_overview.csv"
  )

  cat("\n[probe_QC_overview] Summary written to EPICv1_sesame_probe_QC_overview.csv\n\n")
}
...
cat("========== DNAm QC REPORT ==========\n\n")
cat("Platform: Illumina MethylationEPIC v1.0; processing via SeSAMe;",
...
if (!exists("probe_qc_dt") || !exists("beta_mat_probe_filtered")) {
  ...
  cat("   Input probes (all types):                ", n_input,
      "  [rows in EPICv1_sesame_masked_beta_matrix]\n", sep = "")
  ...
  cat("   Final probes retained after filtering:   ", n_final,
      "  [used in EPICv1_sesame_masked_beta_matrix_probe_filtered]\n\n", sep = "")
}

cat("Full probe-level QC metrics are provided in: ",
    "EPICv1_sesame_probe_QC_table.csv\n\n")
...
drop_candidates <- c(
  ...
  "qc_overall", "qc_by_cohort_region",
